...
This hardware-driven model acts as the core interface for the patient's home environment. It bridges the gap between the recovering patient and their healthcare provider by offering real-time vital sign telemetry, remote medication management, and voice-controlled accessibility. 
...
As the central hub of the patient's room, this model is designed to eventually receive the predictive alerts generated by model 2 (Cardiovascular Risk) and the safety warnings from model 3 (Food Spoilage), creating a fully unified smart healthcare environment.

---

# Model 2: Cardiovascular Risk Prediction using Machine Learning

This software-based model acts as a proactive, predictive safety net. Instead of simply reacting to critical drops in vital signs, it leverages machine learning to forecast a patient's risk of cardiovascular disease or a heart attack, serving as an early-warning decision-support tool for healthcare professionals.
...
*   **Future-Ready Architecture:** Currently operates as an independent software model, with the architecture prepared for future IoT data integration.
...
## 🚀 Future Integration Scope
While currently functioning independently using manually entered data, this model is designed to eventually integrate directly with **model 1**. In future iterations, real-time physiological data (like ECG and heart rate) collected by the ESP32 will automatically feed into this machine learning model to provide continuous, automated, and dynamic risk predictions.

---

# Model 3: Food Spoilage Detection Using AI

Patients recovering in a home-care environment often have compromised immune systems, making them highly vulnerable to foodborne illnesses. This model serves as an intelligent environmental safety mechanism, utilizing multi-sensor data fusion and artificial intelligence to automatically detect food spoilage and ensure the patient's meals are safe to consume.
...
## 🚀 Future Integration Scope
Like model 2, this food spoilage detection system is designed to eventually be integrated into the overarching unified platform. In the future, spoilage alerts could be sent directly to the patient's voice assistant (model 1) or to the remote healthcare provider to ensure dietary safety is maintained alongside physiological health.

--------------------------------------------------------------------------------------------------

# Methodology
The system’s methodology is structured around three independent, specialized models designed to facilitate post-ICU patient recovery at home
. First, an IoT-enabled patient monitoring unit utilizes an ESP32 microcontroller to track essential vitals (including ECG, heart rate, SpO₂, and body temperature) and stream them to a Blynk cloud dashboard where physicians can monitor the data and configure digital prescription reminders
. This model also integrates an ESP32-CAM for live video feed and a ChatGPT-powered voice assistant that lets patients ask questions and control home appliances
. Second, an independent software model assesses cardiovascular risk by evaluating manual clinical inputs—such as age, sex, and cholesterol—using a trained Random Forest machine learning model
. Third, an automated food safety model uses an IR sensor to check for food presence, while gas and temperature sensors (MQ-4, MQ-135, DHT11/22) combined with computer vision detect chemical decay and display color-coded "fresh" or "spoiled" alerts on a web dashboard
. Ultimately, the overarching goal is to integrate these independent systems into a unified platform to establish a comprehensive, smart healthcare ecosystem

## 🧩 Models

## 🩺 Model 1: Intelligent Patient Health Monitoring and Remote Medical Assistance
*Think of this as a smart home-health assistant that connects the patient directly to their doctor.*

*   **Live Vitals Tracking:** Uses attached sensors to continuously monitor heart activity (ECG), heart rate (BPM), blood oxygen (SpO₂), and body temperature.
*   **IoT Integration:** An ESP32 microcontroller collects the sensor data and transmits it securely over Wi-Fi to the **Blynk** cloud platform.
*   **Doctor's Dashboard:** Doctors can log into Blynk from anywhere to view live patient vitals, prescribe medications, and set dosage schedules (morning, afternoon, evening).
*   **Smart Reminders:** The local device receives the doctor's schedule and flashes alerts on a TFT display in the patient's room to remind them to take their medicine.
*   **Telehealth & Voice Assistant:** 
    *   An **ESP32-CAM** streams live video for visual check-ups.
    *   A voice assistant (powered by **ChatGPT**) allows patients to ask health-related questions.
    *   **Smart Room Control:** Patients can use voice commands to control room appliances (like lights and fans) via smart relays.

## 🧠 Model 2: Cardiovascular Risk Prediction Using Machine Learning
*A smart software program that predicts the likelihood of a patient developing heart disease or suffering a heart attack.*

*   **Data Input:** Currently, clinical and demographic data (e.g., age, sex, cholesterol levels) are manually entered into a patient portal.
*   **Machine Learning Brain:** After evaluating multiple algorithms (including Decision Trees and Support Vector Machines), the **Random Forest** algorithm was selected for its superior accuracy in classifying patients into "High Risk" or "Low Risk" categories.
*   **🔮 Future Plan:** The system will soon bypass manual entry, automatically feeding live physiological data (like heart rate and ECG) from model 1 directly into the AI model for real-time risk assessment.

## 🥗 Model 3: Food Spoilage Detection Using AI
*A smart kitchen eye ensuring recovering patients do not consume spoiled food, preventing food poisoning.*

*   **Smart Standby:** An Infrared (IR) sensor checks for the presence of a food tray. If empty, the system enters a low-power standby mode to save energy.
*   **Multi-Sensor Analysis:** When food is detected, an attached camera streams live video while environmental sensors measure:
    *   Temperature & Humidity
    *   Gas levels (specifically detecting methane and decay/rot gases)
*   **Automated Alerts:** The system processes the data in real-time:
    *   *Normal levels:* Displays **"Vegetable is Fresh ✓"**.
    *   *Chemical spike:* Triggers a **"Spoilage Alert"** warning on the digital dashboard, preventing the patient from eating unsafe food.

---

## 🛠️ Technology Stack
*   **Hardware:** ESP32, ESP32-CAM, TFT Display, Smart Relays, IR Sensor, Gas Sensor, Temperature & Humidity Sensors, ECG/BPM/SpO2 Sensors.
*   **Software & Cloud:** Blynk IoT Platform, ChatGPT API.
*   **Machine Learning:** Scikit-Learn (Random Forest, SVM, Decision Trees).

---

## 🚀 Future Roadmap
*   Establish a seamless pipeline bridging model 1 (Live Sensors) directly into model 2 (AI Assessor) for zero-click, 24/7 automated cardiovascular risk prediction.
